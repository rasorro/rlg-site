...
const GAMES: GameCard[] = [
  { title: "RAPID PATH", href: "/rapid-path", imageSrc: "/games/rapid-path/rapidPath.png", gameColor: "var(--color-rapidPath)", isReady: true },
  { title: "STICKMAN SKYDIVE", href: "/stickman-skydive-simulator", imageSrc: "/games/stickman-skydive-simulator/stickmanSkydivePreview.PNG", gameColor: "var(--color-stickman)", isReady: false },
...
function CardAction({ isReady, href, onPlayHoverChange }: CardActionProps) {
  ...
  return isReady ? (
    <TransitionLink href={href} className={`${CARD_ACTION_BASE_CLASS} bg-brand-orange`} onMouseEnter={() => onPlayHoverChange?.(true)} onMouseLeave={() => onPlayHoverChange?.(false)} onFocus={() => onPlayHoverChange?.(true)} onBlur={() => onPlayHoverChange?.(false)}>
      {actionChrome}
    </TransitionLink>
  ) : (
